module.exports = (function() {
     ...
     */
    function getPrefixInfo(propertyName, namespace) {
     ...
     */
    function walk(node, selector, payload, namespaceSelector) {
        node.forEach(function(item, i) {
            var info = getPrefixInfo(
                selector(item),
                namespaceSelector && makeNamespace(namespaceSelector(item))
            );
            if (!info) return;
            payload(info, i);
        });
     ...
     */
    function updateDict(info, dict, whitespaceNode) {
    ...
    return {
        ...
        process: function(value, nodeType, node) {
            ...
            // Gathering Info
            walk(node, getDeclName, function(info, i) {
                updateDict(info, dict, node[i - 1][1]);
            });
            walk(node, getValName, function(info, i) {
                updateDict(info, dict, node[i][3][1][1]);
            }, getDeclName);

            // Update nodes
            walk(node, getDeclName, function(info, i) {
                node[i - 1][1] = updateIndent(info, dict, node[i - 1][1]);
            });
            walk(node, getValName, function(info, i) {
                node[i][3][1][1] = updateIndent(info, dict, node[i][3][1][1]);
            }, getDeclName);
        },
